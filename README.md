...
每个工具完成后，先将完整返回结果、参数和调用 ID 保存到 `.code-agent/context.sqlite3`，再限制模型可见输出；超长结果保留开头及退出码/超时信息，附归档引用和恢复提示。工具内部的旧字符截断在这个执行范围内关闭，读取行数、搜索条数等查询范围限制仍然有效：归档保存本次查询的完整结果，不会额外读取整个文件。归档按会话隔离，追加保存原始消息，不随摘要删除。
...
摘要/掩码只用于模型请求投影，不改写原始 checkpoint 消息。投影单独持久化，恢复会话后继续使用；回滚或修改历史时失效。摘要失败或过大不覆盖旧摘要；最终输入仍超预算时明确报错，不静默删除受保护轮次。归档不自动裁剪，需要自行管理磁盘用量。
...
模型可调用 `search_context_history(query, ref?)` 定位归档内容，再用 `read_context_history(ref, offset_chars, limit_chars)` 按字符范围读取（最多 12000 字符）。这些工具只读取当前会话的归档，不重新执行 shell。定位当前源码仍优先 `search_text`，再 `read_file` 读取局部。结构化摘要使用同配置模型的 JSON 模式并关闭 thinking；本次没有更改主模型重试或调用次数限制。
